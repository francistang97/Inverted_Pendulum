...
int constant_A=2, constant_B=1, constant_C=1, constant_D=2.5;
float CONST_SCALAR_LINEAR_FUNCTION = 0.3;
...
void runInvertedPendulum(){
	...
	prevState.angle = 40* (3.14/180);  
	//to change intial angle change constant above
	...
	while((GetAsyncKeyState(VK_ESCAPE)) == 0 ) {
		...
		//Francis changed here
		//inputs[in_Y] = constant_A* prevState.angle + constant_B*prevState.angle_dot;
		//inputs[in_X] = constant_C*prevState.x + constant_D*prevState.x_dot;

		inputs[in_Y] = CONST_SCALAR_LINEAR_FUNCTION* (constant_A* prevState.angle + constant_B*prevState.angle_dot);
		inputs[in_X] = CONST_SCALAR_LINEAR_FUNCTION* (constant_C*prevState.x + constant_D*prevState.x_dot);			
		 ...
		 externalForce=0.0;
		 externalForce = getKey(); //manual operation - left and right arrow
		 ...
		 //---------------------------------------------------------------------------
		 // **************************************************************************
		 // BEGIN - DYNAMICS OF THE SYSTEM (Physics equations)
	...
	free_fuzzy_rules(&g_fuzzy_system);//include later
}
...
//---------------------------------
    for(int row=0; row < NUM_OF_DATA_POINTS; row++){
         ...
         for(int col=0; col < NUM_OF_DATA_POINTS; col++){
			 ...
			 /*inputs[in_theta] = prevState.angle;
			 inputs[in_theta_dot] = prevState.angle_dot;S
			 inputs[in_x] = prevState.x;
			 inputs[in_x_dot] = prevState.x_dot;*/
	...
	//Update previous state
			 ...
			 inputs[in_Y] = CONST_SCALAR_LINEAR_FUNCTION* (constant_A* prevState.angle + constant_B*prevState.angle_dot);inputs[in_X] = CONST_SCALAR_LINEAR_FUNCTION* (constant_C*prevState.x + constant_D*prevState.x_dot);			
